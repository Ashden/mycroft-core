...
async def on_message(message):
    ...
    if f'<@!{client.user.id}>' == message_split[0]:
        ...
        if len(message_split) == 1:
            ...
            bus.emit(Message('speak', data={'utterance': f'{message.author.name}, did you need something?', 'listen': False}))
        else:
            bus.emit(Message('recognizer_loop:utterance', {"utterances": [command]}, {"discord_message_id": message.id}))
